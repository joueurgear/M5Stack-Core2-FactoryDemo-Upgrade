...
struct systemState
{
    ...
    bool    SDCardState = true;

    uint16_t SDCardscaneCount = 0;

    HotZone_t* App1Zone[7];
    HotZone_t* MPU6886;
...
void AppBluetooth()
{
    ...
    M5.Lcd.drawString("Bluetooth",15,22);
    Disbuff.pushSprite(0,0);
    Disbuff.drawJpg(NewApp,62993,0,0,320,240);
    Disbuff.pushSprite(0,0);
    TouchPoint_t pos = M5.Touch.getPressPoint();
...
void AppMusic()
{
    ...
    M5.Lcd.setTextDatum(CC_DATUM);
    M5.Lcd.setTextFont(2);
    M5.Lcd.drawString("Music",160,74);
    while(1)
        {
            pos = M5.Touch.getPressPoint();
            HotZone Image( 20,191, 64,235);
...
void App()
{
    ...
    M5.Lcd.setTextDatum(CC_DATUM);
    M5.Lcd.setTextFont(2);
    M5.Lcd.drawString("Applications",160,74);
    while(1)
        {
            pos = M5.Touch.getPressPoint();
            HotZone Bluetooth( 16,88, 60,132);
            HotZone Music( 16,133, 60,177);
            HotZone Game( 62,88,106,132);
            ...
            HotZone Setting(256,191,300,235);
            HotZone Home(250,0,294,44);
            if( Bluetooth.inHotZone(pos))
            {   
                AppBluetooth(); break;
            }
            if( Music.inHotZone(pos))
            {   
                AppMusic(); break;
            }
            if( Game.inHotZone(pos))
            {   
                AppGame(); break;
            }
...
void setup()
{
    ...
    sytState.App1Zone[6] = new HotZone(220,0,320,40,&App);
